...
- `railpack.json` is the correct configuration file for Railpack builder (not `railway.json`)
- Railway uses Railpack as its builder, configured via `railpack.json` in the repository root
- Configuration includes:
  - Node.js version: 22
  - Build steps: setup (corepack/yarn), install (yarn install --frozen-lockfile), build (yarn build)
  - Environment variables for VS Code build optimizations:
